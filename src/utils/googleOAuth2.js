...
export const getUsernameFromGoogleTokenPayload = payload => {
  ...
  const nameParts = [];
  if (payload.given_name) nameParts.push(payload.given_name);
  if (payload.family_name) nameParts.push(payload.family_name);

  return nameParts.join(' ') || payload.email?.split('@')[0] || 'Google User';
};
